...
function exportarExcel($reportes) {
    ...
    echo "ID\tAprendiz\tFicha\tPrograma\tMotivo\n";

    foreach ($reportes as $reporte) {
        echo utf8_decode($reporte['idreporte']) . "\t" . utf8_decode($reporte['nombre_aprendiz'] . ' ' . $reporte['apellido_aprendiz']) . "\t" . utf8_decode($reporte['nficha']) . "\t" . utf8_decode($reporte['nombreprograma']) . "\t" . utf8_decode($reporte['motivo']) . "\n";
    }
}
